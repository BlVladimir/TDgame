...
class EnemiesController:
    ...
    def create_enemy(self, context):  # Добавляет в массив врагов новые элементы. В зависимости от количества врагов у каждого врага разные координаты
        ...
        if context.get_config_gameplay().get_waves()[context.get_config_gameplay().get_current_wave()][0] == 1:
            self.__enemy_array.append(EnemyClass.Enemy(image_enemy, context.get_maps_controller().get_started_position(0), scale / 2, health + additional_health, context.get_config_gameplay().get_current_wave()//3, 0, armor=armor, treatment=treatment))
        elif context.get_config_gameplay().get_waves()[context.get_config_gameplay().get_current_wave()][0] == 2:
            self.__enemy_array.append(EnemyClass.Enemy(image_enemy, context.get_maps_controller().get_started_position(1), scale / 2, health + additional_health, context.get_config_gameplay().get_current_wave()//3, 1, armor=armor, treatment=treatment))
            self.__enemy_array.append(EnemyClass.Enemy(image_enemy, context.get_maps_controller().get_started_position(2), scale / 2, health + additional_health, context.get_config_gameplay().get_current_wave()//3, 2,armor=armor, treatment=treatment))
        elif context.get_config_gameplay().get_waves()[context.get_config_gameplay().get_current_wave()][0] == 3:
            self.__enemy_array.append(EnemyClass.Enemy(image_enemy, context.get_maps_controller().get_started_position(2), scale / 2, health + additional_health, context.get_config_gameplay().get_current_wave()//3, 2,armor=armor, treatment=treatment))
            self.__enemy_array.append(EnemyClass.Enemy(image_enemy, context.get_maps_controller().get_started_position(3), scale / 2, health + additional_health, context.get_config_gameplay().get_current_wave()//3, 3,armor=armor, treatment=treatment))
            self.__enemy_array.append(EnemyClass.Enemy(image_enemy, context.get_maps_controller().get_started_position(4), scale / 2, health + additional_health, context.get_config_gameplay().get_current_wave()//3, 4,armor=armor, treatment=treatment))