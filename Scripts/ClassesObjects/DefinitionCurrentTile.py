...
class Highlighting:
    def __init__(self, context):
        self.__highlight_tile_image = pygame.transform.scale(pygame.image.load("images/UI/highlighting/highlightingTower.png"), (context.get_maps_controller().get_tile_scale(), context.get_maps_controller().get_tile_scale()))  # картинка выделения

    def update_scale(self, context):  # обновляет размер выделения(т.к. на разных картах разные размеры тайла)
        self.__highlight_tile_image = pygame.transform.scale(pygame.image.load("images/UI/highlighting/highlightingTower.png"), (context.get_maps_controller().get_tile_scale(), context.get_maps_controller().get_tile_scale()))
