...
def level_controller(event, highlighting, context):
    if event.type == MOUSEBUTTONDOWN:
        context.event_controller.update(context.buttons_groups_controller.action(context), context, highlighting)
    match context.config_parameter_scene.get_scene():
        case 'main_menu':
            if context.config_gameplay.get_waves():  # обнуляет массив врагов и их количество на каждой волне в меню
                ...
                context.enemies_array_controller.clear_enemies_array()
                context.towers_array_controller.clear_towers_arrays()
        case '1' | '2' | '3' | '4' | '5' | '6':
