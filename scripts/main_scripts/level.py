...
def level(event, context):
    context.towers_array_iterator.define_current_tower(context)
    context.enemies_array_iterator.define_current_enemy()
    context.maps_array_iterator.definition_current_tile(event, context)  # определяет текущий тайл
    context.enemies_array_iterator.kill_enemies(context)
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:  # если кнопка мыши нажата
        if context.enemies_array_iterator.get_current_enemy():  # если выделенный враг существует и существует хотя бы одна башня
            context.towers_array_iterator.fire(context)
