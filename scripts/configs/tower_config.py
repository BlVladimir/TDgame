...
from scripts.tower.states_strategy import radius_strategy, gun_strategy
...
class TowerConfig:
    def __init__(self):
        self.__products = {
            'basic_damage': dict(basic_damage=2, cost=3, radius=radius_strategy.RoundRadius(1), improve_cost_array=(4, 6), additional_money=2),
            'sniper': dict(basic_damage=4, cost=5, radius=radius_strategy.RoundRadius(2), improve_cost_array=(6, 8)),
            'anty_shield': dict(basic_damage=3, cost=4, radius=radius_strategy.RoundRadius(1.5), improve_cost_array=(5, 7), armor_piercing=True),
            'venom': dict(basic_damage=2, cost=5, radius=radius_strategy.RoundRadius(1), improve_cost_array=(4, 6), poison=2)}
        ...
        self.__images_bullets_dict = {'poison': BulletWithAnimation(get_images_array_from_directory('animation_poison_bullet')),
                                      ...
                                      'basic': Bullet(pygame.image.load(resource_path('images/tower/Bullets/common_bullet.png')))}
        ...
        self.__sprites_towers_foundations_dict = {
            'basic': OnlyImageSprite(pygame.image.load(resource_path("images/tower/common_foundation.png")), pygame.Rect((0, 0), (0, 0))),
        ...
        self.__sprites_towers_guns_dict = {
            'basic': GunSprite(pygame.image.load(resource_path("images/tower/common_gun.png")), pygame.Rect((0, 0), (0, 0))),
        ...
        self.__visitors_dict = {
            'basic': VisitorForUpgrade(damage=2, radius=1.2),
    ...
    def get_gun_strategy(self, type_tower:str, rect:pygame.Rect):
        if type_tower in self.__sprites_towers_guns_dict.keys():
            return gun_strategy.CommonGunStrategy(self.__sprites_towers_guns_dict[type_tower].clone(rect))
        else:
            return gun_strategy.NullGunStrategy()

    def get_radius_strategy(self, type_tower:str, tile):
        return self.__products[type_tower]['radius'].clone(tile)
