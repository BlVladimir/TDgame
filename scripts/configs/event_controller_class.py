...
def buy_tower(**kwargs):
    if 'additional_image' in kwargs.keys():
        kwargs['context'].towers_array_controller.append_tower_object(
            Tower(kwargs['image'], kwargs['context'].maps_array_controller.get_tile_scale(), kwargs['damage'],
                  kwargs['context'].maps_array_controller.get_build_array()[kwargs['context'].config_gameplay.get_current_tile()]['coordinate'],
...
class EventController:
    ...
    def __create_tower(self, price_coefficient, context, type):  # создает башню с характеристиками, зависящими от текущего тайла
        ...
        match context.maps_array_controller.get_build_array()[context.config_gameplay.get_current_tile()]['type']:
            case 1:
                buy_tower(
            ...
            match event.name:
                ...
                case 'change_scene':
                    ...
                    if event.parameter['scene'].isdigit():
                        context.maps_array_controller.change_level(event.parameter['scene'])
                        ...
                        context.towers_array_controller.update_scale_animation(context)
                        context.maps_array_controller.update_trajectory_array()
                        context.maps_array_controller.create_waves(100, context)  # создает волны
                        context.config_gameplay.set_current_wave(1 - context.config_gameplay.get_current_wave())  # текущая волна
                        context.enemies_array_controller.create_enemy(context)  # создает врагов на 1 клетке
                        ...
                        for i in range(len(context.maps_array_controller.get_build_array())):  # обнуляет все тайлы
                            context.maps_array_controller.get_build_array()[i]['is_filled'] = False
                ...
                case 'buy_tower':
                    ...
                    if not price_up and not is_free and context.config_gameplay.get_money()>= json.loads(self.__products)[event.parameter['type']]['cost']:
                        ...
                        context.maps_array_controller.get_build_array()[context.config_gameplay.get_current_tile()]['is_filled'] = True
                        ...
                        context.maps_array_controller.get_build_array()[context.config_gameplay.get_current_tile()]['is_filled'] = True
                case 'upgrade':
                    if context.towers_array_controller.get_current_tower()and context.towers_array_controller.get_current_tower().get_level() != 3:
                        cost = context.towers_array_controller.get_current_tower().get_improve_cost_array()[context.towers_array_controller.get_current_tower().get_level() - 1]
                        is_free = context.config_modifier.get_is_free()
                        price_up = context.config_modifier.get_price_up()
                        if is_free:
                            context.towers_array_controller.get_current_tower().upgrade(1, 60)
                            context.towers_array_controller.get_current_tower().set_level()
                            context.config_modifier.get_new_value_is_free(False)
                            context.config_modifier.get_new_value_price_up(False)
                            context.towers_array_controller.append_upgrade(context)
