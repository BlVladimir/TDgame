...
class Enemy:
    ...
    def __init__(self, image, rect, scale, angle, health, additional_price, position_on_tile, armor = 0, treatment = 0,pos = 0):
        ...
        self.__animation = []
        self.__x = -1
        files_animation = os.listdir(resource_path('images/enemy/animation_walking'))
    ...
    def move(self, time, context, speed = 100):  # движение врага
        tile_scale = context.maps_array_controller.get_tile_scale(

        )
        if self.__pos // speed != len(context.maps_array_controller.get_trajectory()) - 1:
            difference_position = context.maps_array_controller.get_trajectory()[self.__pos // speed + 1] - context.maps_array_controller.get_trajectory()[self.__pos // speed]
        else:
            difference_position = 0
        if difference_position == 0:
            match context.maps_array_controller.get_trajectory()[self.__pos // speed]:  # сравнивает текущую траекторию
                case 0:
        ...
        if self.__pos // speed == len(context.maps_array_controller.get_trajectory()) and not self.__is_dying:
    ...
    def remove_health(self, context): #  убрать хп
        if context.towers_array_controller.get_current_tower().get_armor_piercing():
            self.__health -= context.towers_array_controller.get_current_tower().get_damage()
        else:
            if context.towers_array_controller.get_current_tower().get_damage() - self.__armor > 0:
                self.__health -= context.towers_array_controller.get_current_tower().get_damage() - self.__armor
        if context.towers_array_controller.get_current_tower().get_poison() != 0:
            if self.__current_damage_poison < context.towers_array_controller.get_current_tower().get_poison():
                self.__treatment -= context.towers_array_controller.get_current_tower().get_poison() - self.__current_damage_poison
                self.__current_damage_poison = context.towers_array_controller.get_current_tower().get_poison()
            self.__poison_dict.append({'damage': context.towers_array_controller.get_current_tower().get_poison(), 'time': 2})
    ...
    def new_value_additional_tower_price(self, context):  # обновляет текущую стоимость врага
        self.__additional_tower_price = context.towers_array_controller.get_current_tower().get_additional_money()
    ...
    def check_dying(self, context):
        if self.__health <= 0:
            context.sound_controller.play_sound('death')
