...
def buy_tower(parameter_dict):  # добавляет в массив башен новую
    ...
    if 'additional_image' in parameter_dict.keys():
        parameter_dict['context'].get_towers_controller().append_tower_object(
            tower_class.Tower(parameter_dict['image'], parameter_dict['context'].get_maps_controller().get_tile_scale(), parameter_dict['damage'],
...
def upgrade_tower(context):  # улучшение башни по номеру
    if context.get_towers_controller().get_current_tower().get_level() != 3:
        cost =  context.get_towers_controller().get_current_tower().get_improve_cost_array()[context.get_towers_controller().get_current_tower().get_level() - 1]
        ...
        if is_free:
            ...
            context.get_towers_controller().get_current_tower().set_level()
            context.get_towers_controller().get_current_button_update().change_image('images/upgrade/2lvl.png') if context.get_towers_controller().get_current_tower().get_level() == 2 \
...
class Product:  # класс продуктов
    ...
    def __create_tower(self, price_coefficient, height, context):  # создает башню с характеристиками, зависящими от текущего тайла
        ...
        context.get_config_gameplay().set_money(-self.__cost * price_coefficient)
    ...
    def buy(self, event,  context):  # покупка башни(возможность покупки и покупка, если возможно)
        ...
        price_up = context.get_config_modifier().get_price_up()
        is_filled = False
        if not price_up and not is_free and self.__button_product.is_pressed(event) and context.get_config_gameplay().get_money() >= self.__cost:
            self.__create_tower(1, context.get_config_parameter_scene().get_height(), context)
            is_filled = True
        elif price_up and not is_free and self.__button_product.is_pressed(event) and context.get_config_gameplay().get_money() >= self.__cost * 2:
